...
template<class T>		//Правая поворот
...
void RBTree<T>::print(RBTNode<T>* node)const {
	if (node == NULL)
		return;
	if (node->parent == NULL)
		cout << node->key << "(" << node->color << ")  корень" << endl;
	else if(node->parent->left==node)
	{
		cout << node->key << "(" << node->color << ")  "<<node->parent->key<<" "<<"левый потомок" << endl;
	}
	else
	{
		cout << node->key << "(" << node->color << ")  " << node->parent->key << " " << "правый потомок" << endl;
	}
	print(node->left);
	print(node->right);
}
